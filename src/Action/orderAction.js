...
try {
    ...
    const config = {
     ...
     const {data}= await axios.post(`https://ecom-server-1ekp.onrender.com/api/order/addorder`,order, config);
      console.log(data)
...
export const listOrder=()=>async (dispatch,getState)=>{
    ...
    const config = {
     ...
     const {data}= await axios.get(`https://ecom-server-1ekp.onrender.com/api/order/getmyorder`, config);
