...
export const productList =
  ...
  async (dispatch) => {
    ...
    try {
      ...
      let url = `https://ecom-server-1ekp.onrender.com/api/product/showproduct?keyword=${keyword}&price[gte]=${price[0]}&price[lte]=${price[1]}&page=${currentPage}`;
      if (categories) {
        url = `https://ecom-server-1ekp.onrender.com/api/product/showproduct?keyword=${keyword}&category=${categories}&price[gte]=${price[0]}&price[lte]=${price[1]}&page=${currentPage}`;
      }
...
export const productDetail = (id) => async (dispatch) => {
  ...
  try {
    await axios.get(`https://ecom-server-1ekp.onrender.com/api/product/${id}`).then((res) => {
...
export const createProductReview = (data) => async (dispatch, getState) => {
  ...
  try {
    ...
    await axios.post(`https://ecom-server-1ekp.onrender.com/api/product/review`, data, config);
