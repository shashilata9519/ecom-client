...
export const login =
  ...
  async (dispatch) => {
    try {
      ...
      await axios
        .post("https://ecom-server-1ekp.onrender.com/api/customer/login", {
          email: email,
          password: password,
        })
        .then((res) => {
...
export const signup = (data) => async (dispatch) => {
  ...
  try {
    ...
    await axios
      .post("https://ecom-server-1ekp.onrender.com/api/customer/register", {
        name: data.fullName,
        email: data.email,
...
export const getUserProfile = () => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get(
      "https://ecom-server-1ekp.onrender.com/api/customer/profile",
      config
    );
...
export const registerWithGoogle = (response) => async (dispatch) => {
  try {
    dispatch({ type: Type.USER_REGISTER_REQUEST });
    await axios
      .post("https://ecom-server-1ekp.onrender.com/api/customer/googleregister", {
...
export const loginWithGoogle = (response) => async (dispatch) => {
  try {
    dispatch({ type: Type.USER_LOGIN_REQUEST });
    await axios
      .post("https://ecom-server-1ekp.onrender.com/api/customer/googlelogin", {
        credential: response.credential,
      })
      .then((res) => {
        dispatch({ type: Type.USER_LOGIN_SUCCESS, payload: res.data });
...
export const updateUserProfile=(response)=>async(dispatch,getState)=>{
    try {
      ...
      const { data } = await axios.put(
        "https://ecom-server-1ekp.onrender.com/api/customer/profile",response,
        config
      );
